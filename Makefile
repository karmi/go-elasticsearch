...
ifdef race
	...
	$(eval testintegargs += "-cover" "-coverprofile=tmp/integration-client.cov" "-tags='integration'" "-timeout=1h" "github.com/karmi/go-elasticsearch/v7" "github.com/karmi/go-elasticsearch/v7/estransport")
	@mkdir -p tmp
	@if which gotestsum > /dev/null 2>&1 ; then \
